...
class BugManagement(models.Model):
    ...
    def action_assign(self):
        ...
        self.activity_schedule(
            ...
            note=_("You have been assigned a bug: %s" % self.name),
        )
        self.message_post(body=_("Bug assigned to %s" % self.assigned_to_id.name))

    def action_in_progress(self):
        self.ensure_one()
        self.state = 'in_progress'
        self.message_post(body=_("Bug is being worked on."))
    ...
    def action_resolve(self):
        ...
        self.message_post(body=_("Bug resolved by %s" % self.closed_by_id.name))
    ...
    def action_close(self):
        ...
        self.solve_date = fields.Datetime.now()
        self.closed_by_id = self.env.user.id
        self.message_post(body=_("Bug closed by %s" % self.closed_by_id.name))
        self.activity_feedback(['mail.mail_activity_data_todo'])

    def action_cancel(self):
        self.ensure_one()
        self.state = 'cancelled'
        self.message_post(body=_("Bug was cancelled."))
